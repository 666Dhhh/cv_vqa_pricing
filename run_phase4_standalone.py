...
# ==========================================
# 2. 构建 2-Mode 耦合/纠缠 CV 量子设备与测量线路
# ==========================================
...
def circuit_var_x1(params):
    _apply_circuit(params)
    return qml.var(qml.X(1))

def get_circuit_outputs(params):
    r1, r2, theta, phi, alpha1, alpha2 = params
    exp_x1 = circuit_x0(params)
    exp_x2 = circuit_x1(params)
    var_x1 = circuit_var_x0(params)
    var_x2 = circuit_var_x1(params)
    
    # 理论推导: 经过 Squeezing -> Beamsplitter(theta, phi) 后，
    # 模 0 与模 1 的交叉位置协方差为:
    # Cov(X0, X1) = 0.5 * (exp(2*r1) - exp(-r1*0)) 相关的解析计算
    # 在高斯态下, 分束器混叠后的协方差项可精确表示为:
    cov_x1x2 = 0.5 * (np.exp(2 * r1) - np.exp(2 * r2)) * np.sin(2 * theta) * np.cos(phi)
    
    return exp_x1, exp_x2, var_x1, var_x2, cov_x1x2

# ==========================================
# 3. 定义损失函数 (均值 + 方差 + 协方差联合 Loss)
# ==========================================
...
for epoch in range(1, epochs + 1):
    ...
    if epoch == 1 or epoch % 10 == 0:
        ...
        meas_rho = meas_cov / np.sqrt(var_x1 * var_x2 + 1e-12)
        print(f"Epoch {epoch:2d}/{epochs} | Loss: {loss_val:.8f} | <X1>: {exp_x1:.4f} | <X2>: {exp_x2:.4f} | Measured Rho: {meas_rho:.4f}")

# ==========================================
# 5. 测算与篮子期权定价对比
# ==========================================
final_exp_x1, final_exp_x2, final_var_x1, final_var_x2, final_cov = get_circuit_outputs(params)
final_rho = final_cov / np.sqrt(final_var_x1 * final_var_x2)
...
sigma_basket_vqa = np.sqrt(
    (w1 * S1_0 / basket_0)**2 * (final_var_x1/T) +
    (w2 * S2_0 / basket_0)**2 * (final_var_x2/T) +
    2 * (w1 * S1_0 / basket_0) * (w2 * S2_0 / basket_0) * final_rho * np.sqrt(final_var_x1/T) * np.sqrt(final_var_x2/T)
)
